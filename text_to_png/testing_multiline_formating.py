...
text = 'This png image was made using python. I built a process that could take text entries, size them correctly, and then generate an image for Google Photos.'
...
for text_size in range(font_size, 0, -1):
    ...
    left, top, right, bottom = draw.multiline_textbbox(center, wrapped, font=font, align='center', spacing=spacing)
...
text_top_left = (center[0] - (instance_text_width / 2) - font_box[0], center[1] - (instance_text_height / 2) - font_box[1])
image = Image.new('RGB', (image_width, image_height), color='black')
draw = ImageDraw.Draw(image)
draw.multiline_text(text_top_left, text_wrapped, font=font, align='center', spacing=spacing, fill=text_color)
# coor = draw.multiline_textbbox(text_top_left, text_wrapped, font=font, align='center', spacing=spacing)
# draw.rectangle(((margin_width, margin_height),
#                (margin_width + text_width, margin_height + text_height)))
# draw.rectangle(((coor[0], coor[1]), (coor[2], coor[3])), fill='red')
# print(coor)
image.save('test_multiline.png')